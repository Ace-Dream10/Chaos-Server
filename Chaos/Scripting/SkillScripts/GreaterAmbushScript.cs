...
public class GreaterAmbushScript : SkillScriptBase
{
    private const int Range = 8;

    /// <inheritdoc />
    public GreaterAmbushScript(Skill subject)
        : base(subject) { }

    /// <inheritdoc />
    public override void OnUse(ActivationContext context)
    {
        //get the points between the source and the max range in the direction the source is facing
        var endPoint = context.Source.DirectionalOffset(context.Source.Direction, Range);
        ...
        foreach (var point in points)
        {
            ...
            if (entity != null)
            {
                ...
                foreach (var direction in behindTargetDirection.AsEnumerable())
                {
                    ...
                    if (!context.TargetMap.IsWalkable(destinationPoint, context.Source, false))
                        continue;
