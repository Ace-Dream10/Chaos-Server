...
public sealed class GroupServiceTests
{
    ...
    public void AcceptInvite_ShouldCloseGroupBox_WhenReceiverJoinsExistingGroup()
    {
        ...
        var leader = MockAisling.Create(
            ...
            setup: a =>
            {
                ...
                a.UserStatSheet.SetBaseClass(BaseClass.Bastion);
            });
    ...
    public void RequestToJoin_ShouldNotRequest_WhenClassLimitReached()
    {
        ...
        var partner = MockAisling.Create(
            ...
            {
                a.Options.AllowGroup = true;
                a.UserStatSheet.SetBaseClass(BaseClass.Mystic);
            });
    ...
    //formatter:off
    [Test]
    [Arguments(BaseClass.Sorcerer)]
    [Arguments(BaseClass.Berserker)] //was WeaponMaster pre-flatten; now falls to the GroupBox _ => 0 default like every other unlisted class - no longer tests a real MaxRogues limit, see class-flatten notes
    [Arguments(BaseClass.Mystic)]
    [Arguments(BaseClass.MartialArtist)]
    ...
    [Test]
    [Arguments(BaseClass.Sorcerer)]
    //Berserker (was WeaponMaster pre-flatten) removed from this case: it now falls to the GroupBox _ => 0 default
    //like every other unlisted class, so "succeeds when under the limit" can never be true for it (0 members is
    //never < a 0 limit) until the real bucket-mapping redesign lands. See class-flatten notes.
    [Arguments(BaseClass.Mystic)]
    [Arguments(BaseClass.MartialArtist)]
    ...
    public void RequestToJoin_ShouldNotRequest_WhenPeasantClassLimit_DefaultsToZero()
    {
        ...
        var sender = MockAisling.Create(
            ...
            {
                a.Options.AllowGroup = true;
                a.UserStatSheet.SetBaseClass(BaseClass.Unregistered);
            });
